...
import { useGetGenreMovies } from '@/services/api/hooks';
...
export const GenreMoviesPage: React.FC<GenreMoviesPageProps> = ({
  genreSlug,
  title,
}) => {
  ...
  const {
    data: genreMoviesData = { data: { items: [] } },
    isLoading,
    refetch,
  } = useGetGenreMovies(genreSlug, {
    page,
    limit: 20,
  ...
  useEffect(() => {
    const items = genreMoviesData?.data?.items || [];
    if (items && items.length > 0) {
      if (page === 1) {
        setAllMovies(items);
      } else {
        setAllMovies((prevMovies) => [...prevMovies, ...items]);
      }
    }
  }, [genreMoviesData, page]);
